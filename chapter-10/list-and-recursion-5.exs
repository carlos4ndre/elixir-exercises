defmodule MyEnum do
  ...
  defp _split(list, n, result), do: { result, list }

  def take(list, n) when n >= 0, do: _take(list, n, [])
  def take(list, n) when n < 0, do: _take(list, n + length(list), [])
  defp _take([head | tail], n, result) when length(result) < n do
    _take(tail, n, result ++ [head])
  end
  defp _take(list, n, result), do: result
end
...
IO.inspect MyEnum.take([1,2,3,4], -1) # [1,2,3]
